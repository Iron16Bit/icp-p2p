...
<script lang='ts'>
   ...
   */
    ...
    function handleStopCooperation(event) {
        event.stopImmediatePropagation();
...
<main>
    {#if visibleButton === 'original'}
        <button 
            id="cooperation-button"
    ...
    {:else if visibleButton === 'new'}
        <button 
        id="cooperation-button"
        on:click={enableCooperationButton} 
        style="position: absolute; right: {type == 'vertical'
        ? `calc(var(--output-height) + min(1.5vw, 3vh))`
        : `min(0.75vw, 1.5vh)`}; 
        top: min(5vw, 10vh); 
    ...
    <dialog on:cancel={closePopup} bind:this={dialogRef}>
        <div class="popup">
            ...
            {#if showConfirmation}
                <p style="white-space: pre-line; text-align: center;">IP: {ipToConfirm}
